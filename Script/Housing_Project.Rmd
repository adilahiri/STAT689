...
fit.ridge <- glmnet(as.matrix(train_subset[, -51]), train_subset$SalePrice,
                    alpha = 0, lambda = grid, thresh = 1e-12)

ridge_mod <- cv.glmnet(as.matrix(train_subset[, -51]), train_subset$SalePrice,
                      alpha = 0, lambda = grid, thresh = 1e-12)

bestlam.ridge <- cv.ridge$lambda.min

pred.ridge <- predict(ridge_mod, s = bestlam.ridge,
                      newx =as.matrix(validation_set[, -51]))
rmse(validation_set$SalePrice, pred.ridge)


ridgeVarImp <- varImp(ridge_mod,scale=F)
Importance <- ridgeVarImp$importance
