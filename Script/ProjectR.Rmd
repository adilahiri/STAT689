...
train_df = read.csv("C:/Users/kenny/Desktop/Project/train.csv", stringsAsFactors = FALSE)
test_df=read.csv("C:/Users/kenny/Desktop/Project/test.csv", stringsAsFactors = FALSE)
```
...
colnames(training_processed)[53]<-"MSZoningC_all"
colnames(testing_processed)[53]<-"MSZoningC_all"

colnames(training_processed)[135]<-"RoofMatlTarGrv"
colnames(testing_processed)[135]<-"RoofMatlTarGrv"

colnames(training_processed)[151]<-"Exterior1stWdSdng"
colnames(testing_processed)[151]<-"Exterior1stWdSdng"

colnames(training_processed)[155]<-"Exterior2ndBrkCmn"
colnames(testing_processed)[155]<-"Exterior2ndBrkCmn"

colnames(training_processed)[167]<-"Exterior2ndWdSdng"
colnames(testing_processed)[167]<-"Exterior2ndWdSdng"

colnames(training_processed)[168]<-"Exterior2ndWdShng"
colnames(testing_processed)[168]<-"Exterior2ndWdShng"
...
lasso_mod <- train(x=train_subset[, -51],
...
lassoImportance[order(-lassoImportance$Overall), , drop = FALSE]
```
...
ridge_mod <- train(x=train_subset[, -51],
                   y=train_subset$SalePrice,
                   method='glmnet', 
...
svm_mod <- train(x=train_subset[, -51],
                   y=train_subset$SalePrice,
                   method='svmLinear', 
                   trControl= my_control_svm, tuneGrid=svmGrid) 
...
svm_predict<- predict(svm_mod, newdata=validation_set[,-51])
...
```{r}
library(tree)
tree.price=tree(SalePrice~.,data=train_subset) #change the dataset
plot(tree.price)
text(tree.price,pretty=0)
cv.tree.price=cv.tree(tree.price)
plot(cv.tree.price$size,cv.tree.price$dev,type='b') 
prune.tree.price=prune.tree(tree.price,best=12) #change the prune number.
plot(prune.tree.price)
text(prune.tree.price,pretty=0)
pred_tree= predict(prune.tree.price, newdata=validation_set)
rmse(validation_set$SalePrice, pred_tree)
...
tab.mtry.df[,2]=tab.mtry.df[,2]^(1/2)
tab.mtry.df[which.min(tab.mtry.df[,2]),1] #The mtry number that yields the lowest RMSE is 130

forest.predict=predict(bag.price,newdata=testing_processed,mtry=130,ntree=150)
forest.predict=as.data.frame(exp(forest.predict))

which(colnames(train_subset)=='Exterior2ndWd Sdng')
```
...
dtrain <- xgb.DMatrix(data = as.matrix(train_subset[,-51]), label= label_train)
dvalid <- xgb.DMatrix(data = as.matrix(validation_set[,-51]))
...
xgbcv <- xgb.cv(params = default_param, data = dtrain, nrounds = 500, 
                nfold = 10, showsd = T, stratified = T, print_every_n = 40,
                early_stopping_rounds = 10, maximize = F)
```

```{r}
xgb_mod <- xgb.train(data = dtrain, params=default_param, nrounds = 454)
XGBpred <- predict(xgb_mod, dvalid)
rmse(validation_set$SalePrice, XGBpred)
```

```{r}
library(Ckmeans.1d.dp)
library(ggplot2)
mat <- xgb.importance (feature_names = colnames(train_subset),model = xgb_mod)
...
```{r}
Decision_Tree_Time <- function(train_subset){
  library(tree)

  tree.price=tree(SalePrice~.,data=train_subset)
  cv.tree.price=cv.tree(tree.price)
...
```{r}
randomforest_time<-function(train_subset){
  bag.price=randomForest(SalePrice~.,data=train_subset,mtry=130,ntree=150)
  rfcv(train_subset[,-51],train_subset[,51],mtry=function(p) max(1, floor(sqrt(p))))
}
```


```{r}
XGBoost_Time <-function(dtrain){
  default_param<-list(
        objective = "reg:linear",
        booster = "gbtree",
        eta=0.05, #default = 0.3
