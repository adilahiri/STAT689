...
```{r}
tree.price=tree(SalePrice~.,data=train_imputed) #change the dataset
plot(tree.price)
text(tree.price,pretty=0)
cv.tree.price=cv.tree(tree.price)
plot(cv.tree.price$size,cv.tree.price$dev,type='b') 
prune.tree.price=prune.tree(tree.price,best=10) #change the prune number.
...
```{r}
tab.mtry=matrix(NA,nrow=ncol(train_imputed)-1,ncol=2)

for(i in 1:(ncol(train_imputed)-1)){
  mtryval=i
  bag.price=randomForest(SalePrice~.,data=train_imputed,mtry=mtryval,ntree=150) #change the dataset
  bag.predict=predict(bag.price) #change the dataset
  tab.mtry[i,1]=i
  tab.mtry[i,2]=mean((bag.predict-train_imputed$SalePrice)^2)
}
